...
export const useRouterStore = defineStore('router', () => {
  ...
  const addRouters = ref<typeof constantRouterMap>([])
  ...
  //根据路由名称获取菜单
  function getMenu(name: string, menus: any[]) {
  ...
  //对菜单进行排序
  function sortRouters(accessedRouters: typeof asyncRouterMap) {
    for (let i = 0; i < accessedRouters.length; i++) {
      let router = accessedRouters[i];
      if (router.children && router.children.length > 0) {
        router.children.sort((a: any, b: any) => b - a);
      }
    }
    accessedRouters.sort((a: any, b: any) => b - a);
  }
