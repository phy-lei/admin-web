...
if (isEdit) {
  getProductAttr($route.query.id).then(response => {
    productAttr = response.data;
    inputListFormat = productAttr.inputList.replace(/,/g, '\n');
  });
} else {
  resetProductAttr();
}
...
function getCateList() {
  ...
  fetchList(listQuery).then(response => {
    productAttrCateList = response.data.list;
  });
}
...
export default {
  ...
  watch: {
    inputListFormat: function (newValue, oldValue) {
      ...
      productAttr.inputList = newValue;
    }
  }
}
</script>
